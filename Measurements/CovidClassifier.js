(function(){function r(e,n,t){function o(i,f){if(!n[i]){if(!e[i]){var c="function"==typeof require&&require;if(!f&&c)return c(i,!0);if(u)return u(i,!0);var a=new Error("Cannot find module '"+i+"'");throw a.code="MODULE_NOT_FOUND",a}var p=n[i]={exports:{}};e[i][0].call(p.exports,function(r){var n=e[i][1][r];return o(n||r)},p,p.exports,r,e,n,t)}return n[i].exports}for(var u="function"==typeof require&&require,i=0;i<t.length;i++)o(t[i]);return o}return r})()({1:[function(require,module,exports){
     ...
     */
    // This module is included in this file using browserify
    var Ngrams = require('simplengrams');
    
    let name = "";
     
    // Variables to store classifier state
     let featureLabels, coefficients, intercepts, tokenizationParams, idfVector = null;
    ...
    onmessage = event => {
        ...
        if (data.type === "init") {    
            ...
            featureLabels = data.args.features;
            coefficients = [data.args.class0_coefs.split(',').map(i => {return parseFloat(i);}),
                            data.args.class1_coefs.split(',').map(i => {return parseFloat(i);}),
                            data.args.class2_coefs.split(',').map(i => {return parseFloat(i);})];
            intercepts = [parseFloat(data.args.class0_intercept),
                          parseFloat(data.args.class1_intercept),
                          parseFloat(data.args.class2_intercept)];
            tokenizationParams = {'title': data.args.title_tokenization_params,
                                  'content': data.args.content_tokenization_params,
                                  'url': data.args.url_tokenization_params};
            idfVector = data.args.tfidf_vector;
        } else if (data.type === "classify") {
            console.log(data.payload.content)
            sendMessageToCaller("classifier ", classify(data.payload), data.payload.url);
        }
    }
     ...
     * @param {string} metadata.content - page content as parsed by the readability script
     * @returns {number} class number
     */
    function classify(metadata, createDebugFile = false) {
        const features = tfidf(getFeaturesFromPage(metadata.url, metadata.title, metadata.content, createDebugFile));
     ...
     */
    function getFeaturesFromPage(url, title, content, createDebugFile) {
        let ngrams = [];
        ...
        for (let i = Number(ngramRange[0]); i <= Number(ngramRange[1]); i++) {
            ngrams = ngrams.concat(Ngrams.ngramsSync(tokens,i).map(t => t.join(" ")).flat());
        }
    ...
    // Code below from browserify and the simplengrams module
    ,{"simplengrams":4}],2:[function(require,module,exports){
    (function (global){
    /**
     * @preserve
     * HappyNodeTokenizer
